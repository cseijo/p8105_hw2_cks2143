...
Changing month number and abbreviation to month name and combining the three datasets.
...
pols_df =
  left_join(pols_df, month538_df, by = "month") %>% 
  select(-month)
  
snp_df =
  left_join(snp_df, month538_df, by = "month") %>% 
  select(-month)
...
final538_df =
  left_join(final538_df, unemployment_df, by = c("year", "month_name"))
